...
/**
 * Publishes the swerve drive state to NetworkTables, the Phoenix log file and the dashboard.
 * telemeterize() is registered as a callback on the drivetrain, so it runs on every odometry update
 * rather than on the 20 ms loop.
 */
public class Telemetry {
  /** Used to normalize module speed into the length of the Mechanism2d ligament. */
  ...
  // Reused buffers so no allocation happens on the odometry thread.
