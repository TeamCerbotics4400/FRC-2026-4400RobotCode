...
/**
 * Physics simulation of the fuel (game piece balls) on the field. Models gravity, drag, bounces off
 * the field geometry, ball to ball collisions, robot collisions, intaking and scoring.
 *
 * <p>Simulation only: it publishes ball positions to NetworkTables so they can be viewed in
 * AdvantageScope. It has no effect on the real robot.
 */
public class FuelSim {
  ...
  // Field geometry as 2D lines in the XZ plane, extruded along Y. Each entry in STARTS pairs with
  // the entry at the same index in ENDS, and together they describe the floor, bumps and trenches.
  ...
  /** One ball: position and velocity in field coordinates, plus its own physics step. */
  protected static class Fuel {
    ...
    /** One physics substep: integrate position, apply gravity and drag, then resolve collisions. */
    protected void update(boolean simulateAirResistance, int subticks) {
      pos = pos.plus(vel.times(PERIOD / subticks));

      // Airborne: gravity and drag apply.
      ...
      // Resting on the ground: stop bouncing and bleed off horizontal speed through friction.
    ...
    /**
     * Bounces the ball off one field line. The problem is reduced to 2D in the XZ plane, and the
     * ball is pushed out along the line normal with the field coefficient of restitution.
     */
    ...
    /** Runs every collision check against static field geometry for this ball. */
    ...
    /** Handles scoring into the hub, bouncing off its sides, and hitting the net above it. */
    ...
    /** Collides the ball against the eight trench blocks and bars, four per alliance side. */
  ...
  /**
   * Elastic collision between two balls: separates them and exchanges momentum along the normal.
   */
  protected static void handleFuelCollision(Fuel a, Fuel b) {
    ...
    // Exactly overlapping balls have no defined normal, so pick an arbitrary one.
    if (distance == 0) {
  ...
  // Spatial hash grid: instead of checking every ball against every other one, balls are bucketed
  // by cell and only the 3x3 neighbourhood is checked. Turns an O(n^2) problem into roughly O(n).
  ...
  /** Cells that hold at least one ball, tracked so clearing does not walk the whole grid. */
  private final ArrayList<ArrayList<Fuel>> activeCells = new ArrayList<>();

  /** Resolves every ball to ball collision using the spatial grid. */
          ...
          if (i >= 0 && i < GRID_COLS && j >= 0 && j < GRID_ROWS) {
            for (Fuel other : grid[i][j]) {
              if (fuel != other && fuel.pos.getDistance(other.pos) < FUEL_RADIUS * 2) {
                // hashCode comparison makes each pair resolve exactly once, not twice.
  ...
  // Set by registerRobot(). While null, robot collisions and intaking are skipped.
  ...
  /** Physics iterations per 20 ms loop. More means more stable collisions but more CPU. */
  ...
  public void spawnStartingFuel() {
    // Center fuel, laid out in a grid spreading outward from the middle of the field
  ...
  public void stepSim() {
    // Each loop is split into substeps so fast balls do not tunnel through walls.
  ...
  public void launchFuel(
    ...
    // Split the launch speed into horizontal and vertical components using the hood angle.
    ...
    // The robot's own velocity adds to the ball, which is what shoot on the move has to cancel.
  ...
  /**
   * Pushes a ball out of the robot footprint. The robot is treated as a rectangle, and the ball is
   * ejected through whichever side it is closest to.
   */
  ...
  /** Applies the robot collision to every ball on the field. */
  ...
  /** Removes from the field every ball that falls inside an active intake area. */
    ...
    for (SimIntake intake : intakes) {
      for (int i = 0; i < fuels.size(); i++) {
        if (intake.shouldIntake(fuels.get(i), robot)) {
          fuels.remove(i);
          // Step back because removing shifts the remaining elements down.
          i--;
        }
      }
    }
  ...
  /**
   * Pushes a ball out of an axis aligned box (hub sides, trench blocks and bars), ejecting it
   * through the nearest face and reflecting its velocity.
   */
  ...
  /** A scoring hub: detects balls dropping in, keeps the score and spits them back out. */
  public static class Hub {
    ...
    /** Where a scored ball reappears on the field. */
    protected final Translation3d exit;

    /** +1 or -1, so each hub ejects balls toward its own side of the field. */
    ...
    /** Scores the ball and teleports it to the exit with a random dispersal velocity. */
    ...
    /**
     * A ball scores when it is inside the entry circle and crossed the entry height downward during
     * this substep. The last term checks the previous position, so a ball travelling upward through
     * the opening does not count.
     */
    ...
    /** Randomized exit velocity so scored balls do not all pile up in the same spot. */
    ...
    /** Bounces balls off the outer walls of the hub, below the entry opening. */
    ...
    /** Returns how far the ball must be pushed back out of the net, or 0 if it did not hit it. */
  ...
  /** A robot relative bounding box that swallows balls while its condition is true. */
  protected class SimIntake {
    /** Bounding box in robot relative coordinates. */
    ...
    /** True when the intake is active and the ball is inside the box, running the callback. */
