...
public class FieldConstants {
  /** Field variant in use. Welded and AndyMark fields differ by a few centimeters. */
  ...
  // Field dimensions, read from the official AprilTag layout instead of being hardcoded
  ...
  /** Hub related constants. This is the structure the robot scores into. */
  public static class Hub {
    ...
    /** Aim point at the top center of the hub, derived from tag 26 plus half the hub width. */
  ...
  /** Tower related constants, including the three climbing rung heights. */
  ...
  /** Depot: the area where extra game pieces are staged. */
  ...
  /** Outpost: the human player station opening on the alliance wall. */
  ...
  /** Field variants, each with its own folder of AprilTag JSON files under deploy/apriltags. */
  ...
  /** AprilTag layouts available on the robot. NONE is used to disable tag localization. */
  public enum AprilTagLayoutType {
    ...
    /** Loads the layout from disk on first use (double checked locking, so it is thread safe). */
    public AprilTagFieldLayout getLayout() {
      if (layout == null) {
        synchronized (this) {
          if (layout == null) {
            try {
              // Off-robot tests read from the source tree, the robot reads from the deploy folder.
              Path p =
