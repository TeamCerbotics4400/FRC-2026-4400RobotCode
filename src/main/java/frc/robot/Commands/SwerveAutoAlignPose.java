...
/**
 * Drives the robot to a fixed field pose using three profiled PID controllers (X, Y and heading).
 * The target pose is picked at initialize() based on the alliance.
 */
public class SwerveAutoAlignPose extends Command {
  ...
  /** Flips the translation output for the red alliance. */
  ...
  public SwerveAutoAlignPose(
      Supplier<Pose2d> redPose, Supplier<Pose2d> bluePose, CommandSwerveDrivetrain m_swerve) {

    // Translation constraints: 3 m/s and 2 m/s^2
    this.xController =
        new ProfiledPIDController(1, 0, 0, new TrapezoidProfile.Constraints(3.0, 2.0));
    this.yController =
        new ProfiledPIDController(1, 0, 0, new TrapezoidProfile.Constraints(3.0, 2.0));

    // Rotation constraints: 360 deg/s and 360 deg/s^2
    ...
    // Wrap around so the robot always turns the short way.
  ...
  /** Picks the alliance target and seeds the profiles with the current pose and velocity. */
  @Override
  ...
  public void execute() {
    ...
    // Profile velocity is used as feedforward, the PID output only corrects the error.
    ...
    // Zero out each axis once it is close enough, to stop the modules from buzzing.
    ...
    if (Math.abs(currentPose.getRotation().minus(targetPose.get().getRotation()).getDegrees())
        < 0.01) {
      rotVel = 0;
    }
  ...
  /** Never ends on its own, the caller decides when to stop aligning. */
  @Override
